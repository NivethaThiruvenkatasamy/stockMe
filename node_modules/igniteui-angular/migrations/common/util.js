    ...
    try {
        ...
        switch (packageManager) {
            case 'yarn':
                ...
                break;
            case 'npm':
                child_process_1.execSync(`${packageManager} i ${pkg} --no-save --no-audit`, { stdio: 'ignore' });
                break;
        }
 ...
 * @param encoding
 */
const parseFile = (host, filePath, encoding = 'utf8') => new compiler_1.HtmlParser().parse(host.read(filePath).toString(encoding), filePath).rootNodes;
...
const serializeNodes = (nodes) => nodes.map(node => node.visit(new SerializerVisitor(), null));

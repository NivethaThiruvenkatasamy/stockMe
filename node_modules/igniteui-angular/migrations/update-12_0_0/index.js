...
See https://www.infragistics.com/products/ignite-ui-angular/angular/components/time-picker -->\n`
    ...
    for (const path of htmlFiles) {
        util_1.findElementNodes(util_1.parseFile(host, path), 'igx-tabs')
            .forEach(node => {
            if (util_1.hasAttribute(node, 'type')) {
                const { startTag, file } = util_1.getSourceOffset(node);
                const tabsType = util_1.getAttribute(node, 'type')[0];
                let alignment;
    ...
    for (const comp of COMPONENTS) {
        for (const path of htmlFiles) {
            ...
            util_1.findElementNodes(util_1.parseFile(host, path), comp.tags)
                .map(tab => util_1.findElementNodes([tab], 'ng-template'))
                .reduce((prev, curr) => prev.concat(curr), [])
                .filter(template => util_1.hasAttribute(template, 'igxTab'))
                .forEach(node => {
            ...
            util_1.findElementNodes(util_1.parseFile(host, path), comp.tags).
                ...
                // Label content
                let labelText = '';
                if (util_1.hasAttribute(node, 'label')) {
                    const labelAttr = util_1.getAttribute(node, 'label')[0];
                    labelText = `\n<span ${comp.labelDirective}>${labelAttr.value}</span>`;
                }
                // Icon content
                let iconText = '';
                if (util_1.hasAttribute(node, 'icon')) {
                    const iconAttr = util_1.getAttribute(node, 'icon')[0];
                    iconText = `\n<igx-icon ${comp.iconDirective}>${iconAttr.value}</igx-icon>`;
                }
                // RouterLink
                let routerLinkText = '';
                if (util_1.hasAttribute(node, 'routerLink')) {
                    const routerLink = util_1.getAttribute(node, 'routerLink')[0];
                    routerLinkText = ` ${routerLink.name}="${routerLink.value}"`;
                }
                let classText = '';
                if ((node.name === 'igx-tab-item' || node.name === 'igx-tab') && util_1.hasAttribute(node, 'class')) {
                    const classAttr = util_1.getAttribute(node, 'class')[0].value;
                ...
                if (iconText || labelText || routerLinkText) {
                    ...
                    const headerNode = util_1.findElementNodes([offset.node], comp.headerItem)
                        .map(hNode => util_1.getSourceOffset(hNode));
            ...
            util_1.findElementNodes(util_1.parseFile(host, path), comp.tags)
                ...
                if (offset.node.name === 'igx-tab-panel' || offset.node.name === 'igx-tabs-group') {
                    const classAttr = util_1.hasAttribute(offset.node, 'class') ? util_1.getAttribute(offset.node, 'class')[0].value : '';
            ...
            if (applyComment) {
                util_1.findElementNodes(util_1.parseFile(host, path), comp.component).
                    map(node => util_1.getSourceOffset(node)).
                    forEach(offset => {
    ...
    for (const comp of EDITOR_COMPONENTS) {
        for (const path of htmlFiles) {
            ...
            util_1.findElementNodes(util_1.parseFile(host, path), comp.COMPONENT)
                .map(editor => util_1.findElementNodes([editor], 'ng-template'))
                .reduce((prev, curr) => prev.concat(curr), [])
                .filter(template => util_1.hasAttribute(template, comp.TEMPLATE_DIRECTIVE))
                .map(node => util_1.getSourceOffset(node))
                .forEach(offset => {
                const { startTag, file } = offset;
            ...
            util_1.findElementNodes(util_1.parseFile(host, path), comp.COMPONENT)
                .filter(template => util_1.hasAttribute(template, EDITORS_MODE))
                .map(node => util_1.getSourceOffset(node))
                .forEach(offset => {
                const { file } = offset;
                util_1.getAttribute(offset.node, EDITORS_MODE).forEach(attr => {
            ...
            util_1.findElementNodes(util_1.parseFile(host, path), comp.COMPONENT)
                .filter(template => util_1.hasAttribute(template, EDITORS_LABEL))
                .map(node => util_1.getSourceOffset(node))
                .forEach(offset => {
                const { startTag, file } = offset;
                let visibilityValue = true;
                if (util_1.hasAttribute(offset.node, EDITORS_LABEL_VISIBILITY)) {
                    const visibility = util_1.getAttribute(offset.node, EDITORS_LABEL_VISIBILITY);
                    visibilityValue = visibility[0].value;
                }
                util_1.getAttribute(offset.node, EDITORS_LABEL).forEach(attr => {
            ...
            util_1.findElementNodes(util_1.parseFile(host, path), comp.COMPONENT)
                .filter(template => !util_1.hasAttribute(template, EDITORS_LABEL) &&
                !util_1.hasAttribute(template, EDITORS_LABEL_VISIBILITY))
                .map(node => util_1.getSourceOffset(node))
                .forEach(offset => {
                if (util_1.findElementNodes([offset.node], 'label').length === 0) {
                    const { startTag, file } = offset;
    ...
    for (const path of htmlFiles) {
        const inputGroups = util_1.findElementNodes(util_1.parseFile(host, path), INPUT_GROUP_CHANGES.GROUP_TAG);
        inputGroups.forEach((el) => {
            const parentEl = el;
            if (util_1.hasAttribute(parentEl, INPUT_GROUP_CHANGES.ATTRIBUTES)) {
                const inputChildren = util_1.findElementNodes([el], INPUT_GROUP_CHANGES.INPUT_TAG)
                    .reduce((prev, curr) => prev.concat(curr), [])
                    .filter(template => util_1.hasAttribute(template, INPUT_GROUP_CHANGES.DIRECTIVE));
                INPUT_GROUP_CHANGES.ATTRIBUTES.forEach((a) => {
                    const attr = util_1.getAttribute(parentEl, a)[0];
                    if (attr) {
                        inputChildren.forEach((node) => {
                            if (!util_1.hasAttribute(node, INPUT_GROUP_CHANGES.ATTRIBUTES)) {
                                const { startTag, file } = util_1.getSourceOffset(node);
